...
export function CadCentroCusto() {
    const [codigo, setCodigo] = useState('');
    const [descricao, setDescricao] = useState('');
    const [value, setValue] = useState(0);
    const [refreshCentroCusto, setRefreshCentroCusto] = useState(false);
        ...
        },
        {
            ...
            renderCell: (params) => {
                return (
                    ...
                    >
                        <MatRemove index={params.row.id} table={'centroCusto'} refresh={() => { setRefreshCentroCusto(!refreshCentroCusto) }} />
                    </div>
                );
            }
        },
    ...
    useEffect(() => {
        async function buscaItens() {
            ...
            let { data, error } = await supabase.from('centroCusto').select().order('id', { ascending: true })
    ...
    async function handleSalvar(event) {
        ...
        setLoading(true);
        let retorno = await supabase.from('centroCusto').upsert(registro)
    ...
    return (
        <div className="containerCadastro" >
            <TopTitle title="Centro de Custo"></TopTitle>
